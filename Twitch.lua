local Scriptname,Version,Author,LVersion = "TRUSt in my Twitch","v1.1","TRUS","7.6"
...
function Twitch:UseERange()
	...
	for i, hero in pairs(heroeslist) do
		if stacks[hero.charName] and self:GetStacks(stacks[hero.charName].name) >= self.Menu.MinStacks:Value() then
			...
			if myHero.pos:DistanceTo(hero.pos)<1100 and myHero.pos:DistanceTo(hero:GetPrediction(math.huge,0.25).pos) > 900 then
				useE = true
			end
		end
	end
